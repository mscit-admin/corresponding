...
المتطلب الوحيد هو **Docker** و **Docker Compose**. أمر واحد يشغّل قاعدة البيانات والواجهة الخلفية والأمامية معاً:
...
- تشغيل قاعدة بيانات MySQL 8.
- تطبيق مخطط قاعدة البيانات (`prisma db push`) وإدخال البيانات الأولية (seed).
- بناء وتشغيل الـ API والواجهة.

بعد اكتمال التشغيل:

| الخدمة | الرابط |
|--------|--------|
| الواجهة (Frontend) | http://localhost:3300 |
| الـ API (Backend) | http://localhost:3100 |
| توثيق Swagger | http://localhost:3100/api/docs |
| MySQL | localhost:3306 |

**تسجيل الدخول:** المستخدم `admin` / كلمة المرور `Admin@1234`

أوامر مفيدة:

```bash
docker compose up --build -d     # التشغيل في الخلفية
docker compose logs -f backend   # متابعة سجلّات الـ API
docker compose down              # إيقاف الحاويات
docker compose down -v           # إيقاف وحذف البيانات (قاعدة البيانات والمرفقات)
```

> ملاحظة: المرفقات (بما فيها المستندات الممسوحة بالسكانر) تُحفظ في volume باسم `uploads_data`، وبيانات قاعدة البيانات في `db_data`، لذا تبقى محفوظة بين عمليات إعادة التشغيل.

> لتغيير رابط الـ API المضمّن في الواجهة، عدّل `NEXT_PUBLIC_API_URL` ضمن `build.args` لخدمة `frontend` في `docker-compose.yml` ثم أعد البناء.
...
- ✅ **لجنة التخصيص** (تخصيص الأراضي): دورة كاملة من الورود حتى صدور قرار التخصيص

### 🏛️ وحدة لجنة التخصيص (لجنة مشتركة بين مصلحة أملاك الدولة ورئاسة الوزراء)

تتيح الوحدة متابعة طلبات تخصيص الأراضي عبر دورة العمل الكاملة مع الاحتفاظ بكامل البيانات (المقبولة والمرفوضة وقيد الدراسة):

1. **تسجيل الطلب** برقم تسلسلي ورقم أسبقية، وإنشاء قائمة المستندات المطلوبة تلقائياً:
   رسم كروكي معتمد من التخطيط العمراني، شهادة عقارية باسم الدولة، تقرير مفصل من واقع الطبيعة،
   وموافقة وزارة الزراعة (إذا كان الموقع خارج المخطط).
2. **العرض على اللجنة**؛ وعند وجود نواقص تُسجّل وتُراسَل الجهة لاستيفائها وحفظ صورة الكتاب في الملف.
3. **قرار اللجنة** (موافقة/عدم موافقة) — لا يُسمح بالموافقة قبل استيفاء كل المستندات المطلوبة.
4. **إدراج الطلب في محضر اللجنة** (حتى 12 بنداً لكل محضر).
5. **اعتماد المحضر من مجلس الوزراء**: تتحول الطلبات الموافَق عليها إلى «صدور قرار تخصيص وتحويل للمكتب المختص»،
   وغير الموافَق عليها إلى «مرفوض وإبلاغ المكتب المختص».
6. **سجل إجراءات (Timeline)** كامل لكل طلب لحفظ التاريخ والبيانات.
