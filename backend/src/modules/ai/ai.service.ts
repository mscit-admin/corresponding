...
// حدّ مخرجات النموذج — كبير بما يكفي ليحتوي JSON كاملاً مع ملخّص مطوّل وتفريغ نصّي حرفي
...
/** تعليمة تُلحَق بنص التوجيه لضمان إرجاع تفريغ خام حرفي لكامل المستند. */
const FULLTEXT_INSTRUCTION = `

مهم: أضِف إلى كائن JSON حقلاً إضافياً اسمه "fullText" قيمته تفريغ نصّي خام وحرفي لكامل ما هو مكتوب في المستند كما هو تماماً (دون تلخيص ودون إعادة صياغة ودون إضافة عناوين أو شرح)، مع الحفاظ على ترتيب الأسطر قدر الإمكان. يجب أن يكون الردّ كائن JSON واحداً صالحاً يحوي الحقول: subject و summary و fullText و confidence.`;
...
export class AiService {
  ...
  async extractSubject(
    ...
    const basePrompt = (await this.getRaw(K_PROMPT)) || DEFAULT_PROMPT;
    const prompt = basePrompt + FULLTEXT_INSTRUCTION;
  ...
  private parse(raw: string): SubjectExtraction {
    ...
    try {
      ...
      return {
        ...
        fullText: String(p.fullText || '').trim(),
        confidence: conf,
      };
    } catch {
      ...
      const fullText = this.extractField(text, 'fullText');
      if (subject || summary || fullText) {
        return { subject, summary: summary || raw.slice(0, 500), fullText, confidence: 'low' };
      }
      return { subject: '', summary: raw.slice(0, 500), fullText: '', confidence: 'low' };
    }
  }
