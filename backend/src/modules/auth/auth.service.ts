...
export class AuthService {
  ...
  async login(dto: LoginDto, ipAddress: string, userAgent?: string, deviceMac?: string, deviceHost?: string, deviceId?: string): Promise<LoginResponseDto> {
    ...
    if (user.role.name !== 'super_admin') {
      const ctx: DeviceContext = { ipAddress, userAgent, deviceMac, deviceHost, deviceId };
      const companyDevice = await this.externalAccess.isCompanyIp(ipAddress);
      if (companyDevice) {
        // داخل شبكة المؤسسة: اعتماد الجهاز كالمعتاد
        await this.enforceTrustedDevice(user.id, user.fullName, ctx);
      } else {
        // خارج الشبكة: يتطلّب موافقة مدير النظام لهذا الجهاز (بمدة/مفتوحة) أو قفل
        await this.externalAccess.enforceAtLogin(user, ctx);
      }
    }
